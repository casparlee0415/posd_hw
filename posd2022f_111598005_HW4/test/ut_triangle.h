...
TEST_F(TriangleTest,testCreateDFSIterator){
    ...
    ASSERT_ANY_THROW(triangle->createIterator(IteratorFactory::getInstance("DFS"))->first());
}

TEST_F(TriangleTest,testCreateBFSIterator){
    
    ASSERT_ANY_THROW(triangle->createIterator(IteratorFactory::getInstance("BFS"))->first());
}
