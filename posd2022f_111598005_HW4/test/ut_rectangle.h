...
TEST_F(RectangleTest, testCreateDFSIterator) {
    ...
    ASSERT_ANY_THROW(rectangle->createIterator(IteratorFactory::getInstance("DFS"))->first());
}

TEST_F(RectangleTest, testCreateBFSIterator) {

    ASSERT_ANY_THROW(rectangle->createIterator(IteratorFactory::getInstance("BFS"))->first());
}
