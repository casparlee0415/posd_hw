...
TEST_F(CircleTest,testCreateDFSIterator){
    ...
    ASSERT_ANY_THROW(circle->createIterator(IteratorFactory::getInstance("DFS"))->first());
}

TEST_F(CircleTest,testCreateBFSIterator){
    
    ASSERT_ANY_THROW(circle->createIterator(IteratorFactory::getInstance("BFS"))->first());
}
